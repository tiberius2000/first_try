...
if [[ $SRC_REPO = "TEST" ]] then
        SRC_UID="TEST_PMREP"
        SRC_PWD=I1KGf3tqArBCkKbZmYx/b9qA3Rb84/3DZn1zhkURpjI=
...
#	SRC_PWD="vct_pwx39"
	...
	SRC_UID="VCT_PMREP"
	SRC_PWD=4biXzsAdV4iHOuXBk98R+Q==
else
	SRC_UID=""
	SRC_PWD=""
fi
...
if [[ $TGT_REPO = "TEST" ]] then
	TGT_UID="TEST_PMREP"
	TGT_PWD=I1KGf3tqArBCkKbZmYx/b9qA3Rb84/3DZn1zhkURpjI=
...
#	TGT_PWD="vct_pwx39"
	...
	TGT_UID="VCT_PMREP"
	TGT_PWD=4biXzsAdV4iHOuXBk98R+Q==
else
	TGT_UID=""
	TGT_PWD=""
fi
...
do
	...
	if [[ $REC_CNT -eq 0 ]] then
		echo "\nNone of the records in the DeploymentObjects.dat file meet the criteria to be processed..." >> $LOG_FILE_NAME
		exit 0
	fi
		...
		GROUP_NAME="dpl_REL${Release}_QC${QC}_${CUR_TIMESTAMP}"
				...
				for i in ${fail[@]}
					...
					if [[ $PROCESS_FLG = "TRUE" ]] then
						#echo "PROCESSING"
						pmrep listobjectdependencies -n $ObjectName -o $ObjectType -p children -f "$Folder" -u $TMP_FILE_LOC/ObjectDep.lst >> $ LOG_FILE_NAME
						
						
						#if [[ $ObjectType = "Task" ]] then
						#pmrep listobjects -o $ObjectType -f "$Folder" -u $TMP_FILE_LOC/ObjectTask.lst >> $ LOG_FILE_NAME
						#fi
						...
						pmrep applylabel -a $LABEL_NAME -i $TMP_FILE_LOC/ObjectDep.lst >> $LOG_FILE_NAME
						
						if [[ $ObjectType = "Task" ]] then
						pmrep applylabel -a $LABEL_NAME -n $ObjectName -o $ObjectType -t email -f "$Folder"
						fi
						...
						pmrep addtodeploymentgroup -p $GROUP_NAME -i $TMP_FILE_LOC/ObjectDep.lst >> $LOG_FILE_NAME
						
						if [[ $ObjectType = "Task" ]] then
						pmrep addtodeploymentgroup -p $GROUP_NAME -n $ObjectName -o $ObjectType -t email -f "$Folder"
...
TGT_LABEL_NAME=$2
  ...
  echo '   COPYDEPENDENCY="YES"' >> $XMLFILE
	...
	for i in ${group[@]}
		do
			if [[ $DeployGroup = $i ]] then
				...
				PWD=$TGT_PWD
				pmrep deploydeploymentgroup -p $DeployGroup -r $TGT_REPO -c $XMLFILE -h $TGT_HOST -o $TGT_PORT -n $TGT_UID -X PWD >> $LOG_FILE_NAME
...
LOG_FILE_NAME="$LOG_FILE_LOC/${CUR_TIMESTAMP}_AUTO_DEPLOY_${SRC_REPO}_TO_${TGT_REPO}.log"
...
if [[ -f $DEPLOY_FILE_LOC/$DEPLOY_FILE_NME ]] then
	if [[ -r $DEPLOY_FILE_LOC/$DEPLOY_FILE_NME ]] then
		...
		#Deploy deployment groups from source repository torao target repository
		DeployDeploymentGroup 

		cp $

		mv $DEPLOY_FILE_LOC/$DEPLOY_FILE_NME $DEPLOY_FILE_ARCH_LOC/${CUR_TIMESTAMP}_DeploymentObjects.dat
